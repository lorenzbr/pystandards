...
class ieee_contributions:
    ...
    def init_df_output(self, index=None):
        ...
        columns = ['date_time_created', 'year', 'dcn', 'rev',
                   'group', 'title', 'author', 'date_time_upload', 'dl_link']
    ...
    def get_df_output(self, stdname, page):
        ...
        for entry in entries:
            ...
            file = entry.find_all('td', class_='list_actions')[
                0].find('a')['href']

            df_results = pd.DataFrame({"date_time_created": date_time_created,
                                       "year": year, "dcn": dcn, "rev": rev,
                                       "group": group, "title": title,
                                       "author": author,
                                       "date_time_upload": date_time_upload,
                                       "file": file}, index=['IDX'])
    ...
    def get_meta(self, std_name, start_page, end_page):
        ...
        for page in range(start_page, end_page + 1):
            ...
            print("Page " + str(page) + " Time: " +
                  str(datetime.datetime.now()))

        # get document type
        df_output['doc_type'] = [
            re.split(r"\.(?=[^.]*$)", x)[-1] for x in df_output['file']]

        # link that can be used to download contributions
        df_output['dl_link'] = self.url + df_output['file']

        df_output.loc[:, 'file'] = [
            re.sub(r"^/", "", x) for x in df_output['file']]
        df_output.loc[:, 'file'] = [
            re.sub(r"\.([^.]*$)", "", x) for x in df_output['file']]
        df_output.loc[:, 'file'] = [
            re.sub(r"/|\.", "_", x) for x in df_output['file']]
